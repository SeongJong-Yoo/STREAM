# Evaluator Wrapper for HumanML3D mainly from https://github.com/neu-vi/ACMDM
import torch
...
from src.dataloader.glove import POS_enumerator


#################################################################################
#                                    Evaluators                                 #
#################################################################################
def build_evaluators(dim_pose, dim_movement_enc_hidden, dim_movement_latent, dim_word, dim_pos_ohot, dim_text_hidden,
                     dim_coemb_hidden, dim_motion_hidden, device):
    ...
    checkpoint = torch.load(os.path.join('./src/metric/checkpoints', 'text_mot_match', 'model', 'finest.tar'),
                            map_location=device)
    checkpoint_clip = torch.load(os.path.join('./src/metric/checkpoints', 'text_mot_match_clip', 'model', 'finest.tar'),
                            map_location=device)
...
class Evaluators(object):
    ...
    def __init__(self, device):
        ...
        = build_evaluators(dim_pose, dim_movement_enc_hidden, dim_movement_latent, dim_word,
                            dim_pos_ohot, dim_text_hidden, dim_coemb_hidden, dim_motion_hidden, device)
        self.device = device
